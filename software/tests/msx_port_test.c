...
static void expect_outputs(joystick_state_t state)
{
    for (unsigned index = 0; index < 6; ++index) {
        assert(outputs[6 + index] == ((state & (1u << index)) != 0));
        assert(pulls_disabled[6 + index]);
    }
}
...
int main(void)
{
    ...
    unsigned previous_changes = direction_changes[10];
    ...
    assert(direction_changes[10] == previous_changes);
